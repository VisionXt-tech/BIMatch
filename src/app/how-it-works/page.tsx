...
import { ArrowRight, UserPlus, Search, MessageSquare, Briefcase, FileText, Users, Brain, Zap, MapPin, CheckCircle, Building } from "lucide-react";
import Link from "next/link";
import ScrollFloat from "@/components/ScrollFloat";
import CardReveal from "@/components/CardReveal";
import React from "react";

const StepCard = ({ icon: Icon, title, description, imageSrc }: { icon: React.ElementType, title: string, description: string, imageSrc: string }) => (
  <Card className="shadow-md hover:shadow-xl transition-all duration-300 flex flex-col h-full bg-white border border-gray-200 group overflow-hidden rounded-xl">
    <div className="relative aspect-[16/9] overflow-hidden">
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-5 2xl:p-6">
        <div className="flex items-center text-white">
          <div className="bg-white/20 backdrop-blur-sm p-2 sm:p-2.5 2xl:p-3 rounded-lg mr-2 sm:mr-3 flex-shrink-0">
    ...
    <CardContent className="flex-grow p-4 sm:p-5 md:p-6 2xl:p-7">
      <p className="text-sm sm:text-base 2xl:text-lg text-gray-600 leading-relaxed">{description}</p>
    </CardContent>
  </Card>
);

const BenefitItem = ({ icon: Icon, title, description }: { icon: React.ElementType, title: string, description: string }) => (
  <div className="flex flex-col items-center text-center p-5 sm:p-6 2xl:p-8 bg-white rounded-xl border border-gray-200 shadow-md hover:shadow-xl transition-all duration-300 h-full">
    <div className="flex-shrink-0 mb-3 sm:mb-4 2xl:mb-5 bg-primary/10 p-3 sm:p-4 2xl:p-5 rounded-full">
      <Icon className="h-6 w-6 sm:h-7 sm:w-7 2xl:h-8 2xl:w-8 text-primary" />
    </div>
    <h4 className="font-bold text-base lg:text-lg text-gray-900 mb-2 2xl:mb-3 leading-tight hyphens-manual break-normal">{title}</h4>
    <p className="text-sm sm:text-base 2xl:text-lg text-gray-600 leading-relaxed">{description}</p>
  </div>
);
...
const ProcessStep = ({ icon: Icon, number, title, description }: { icon: React.ElementType, number: number, title: string, description: string }) => (
  <div className="sticky top-24 bg-white rounded-xl md:rounded-2xl border border-gray-200 shadow-lg p-6 sm:p-8 md:p-10 lg:p-12 2xl:p-14 mb-6 md:mb-8 2xl:mb-10">
    <div className="flex flex-col md:flex-row items-center md:items-start gap-4 md:gap-6 2xl:gap-8">
      <div className="flex-shrink-0">
        <div className="relative">
          <div className="bg-primary/10 p-4 sm:p-5 md:p-6 2xl:p-7 rounded-xl md:rounded-2xl">
            <Icon className="h-8 w-8 sm:h-10 sm:w-10 md:h-12 md:w-12 2xl:h-14 2xl:w-14 text-primary" />
          </div>
          <div className="absolute -top-2 -right-2 bg-primary text-white w-7 h-7 sm:w-8 sm:h-8 2xl:w-9 2xl:h-9 rounded-full flex items-center justify-center text-xs sm:text-sm 2xl:text-base font-bold">
            {number}
          </div>
        </div>
      </div>
      <div className="flex-1 text-center md:text-left">
        <h3 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 mb-2 md:mb-3 2xl:mb-4 leading-tight hyphens-manual break-normal">{title}</h3>
        <p className="text-base sm:text-lg 2xl:text-xl text-gray-600 leading-relaxed">{description}</p>
      </div>
    </div>
  </div>
);
...
export default function HowItWorksPage() {
  ...
  return (
    <div className="bg-white min-h-screen">
      {/* Hero Section */}
      <section className="py-16 sm:py-20 md:py-28 2xl:py-32 text-center">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 2xl:px-16">
          <div className="max-w-5xl 2xl:max-w-6xl mx-auto">
            <ScrollFloat
              containerClassName="mb-6 2xl:mb-8"
              textClassName="text-3xl sm:text-4xl lg:text-5xl font-bold text-gray-900 leading-snug hyphens-none break-words"
              animationDuration={1}
              ease="power2.out"
            ...
            <p className="text-base sm:text-lg md:text-xl 2xl:text-2xl text-gray-600 max-w-3xl 2xl:max-w-4xl mx-auto mb-8 md:mb-10 2xl:mb-12 px-4">
              La piattaforma che connette i migliori professionisti BIM con le aziende leader del settore
            </p>

            <Button size="lg" asChild className="bg-primary hover:bg-primary/90 shadow-lg 2xl:text-lg 2xl:px-8 2xl:py-6">
              <Link href={ROUTES.REGISTER_PROFESSIONAL}>
                Inizia Ora <ArrowRight className="ml-2 h-5 w-5 2xl:h-6 2xl:w-6" />
              </Link>
            </Button>
          </div>
        </div>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 2xl:px-16">
          <div className="text-center mb-12 md:mb-16 2xl:mb-20 max-w-4xl 2xl:max-w-5xl mx-auto">
            <ScrollFloat
              ...
              textClassName="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-900 leading-snug hyphens-none break-words"
              animationDuration={1}
              ease="power2.out"
              scrollStart="top bottom"
              scrollEnd="center center"
            ...
            <p className="text-base sm:text-lg md:text-xl 2xl:text-2xl text-gray-600 px-4">Dal primo accesso alla collaborazione di successo</p>
          </div>
      ...
      <section className="py-16 sm:py-20 lg:py-24 2xl:py-32 bg-white">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 2xl:px-16">
          <div className="text-center mb-12 md:mb-16 2xl:mb-20 max-w-4xl 2xl:max-w-5xl mx-auto">
            ...
            <p className="text-base sm:text-lg md:text-xl 2xl:text-2xl text-gray-600 px-4">Trova progetti che valorizzano le tue competenze</p>
          </div>
      ...
      <section className="py-16 sm:py-20 lg:py-24 2xl:py-32 bg-white">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 2xl:px-16">
          <div className="text-center mb-12 md:mb-16 2xl:mb-20 max-w-4xl 2xl:max-w-5xl mx-auto">
            ...
            <p className="text-base sm:text-lg md:text-xl 2xl:text-2xl text-gray-600 px-4">Accedi ai migliori talenti BIM in Italia</p>
          </div>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 2xl:px-16">
          <div className="text-center mb-12 md:mb-16 2xl:mb-20 max-w-4xl 2xl:max-w-5xl mx-auto">
            <ScrollFloat
              ...
              stagger={0.02}
            >
              Perché Scegliere BIMatch
            </ScrollFloat>
            <p className="text-base sm:text-lg md:text-xl 2xl:text-2xl text-gray-600 px-4">La piattaforma pensata per il settore BIM italiano</p>
          </div>
      ...
      <section className="py-16 sm:py-20 md:py-28 2xl:py-36 bg-white">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 2xl:px-16 text-center">
          <div className="max-w-4xl 2xl:max-w-5xl mx-auto">
            ...
            <p className="text-base sm:text-lg md:text-xl 2xl:text-2xl text-gray-600 mb-8 md:mb-10 2xl:mb-12 max-w-3xl 2xl:max-w-4xl mx-auto px-4">
              Unisciti alla community BIM italiana e scopri nuove opportunità
            </p>

            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 2xl:gap-6 px-4">
              <Button asChild size="lg" className="bg-primary hover:bg-primary/90 shadow-lg w-full sm:w-auto min-w-[200px] 2xl:min-w-[240px] 2xl:text-lg 2xl:px-8 2xl:py-6">
                <Link href={ROUTES.REGISTER_PROFESSIONAL}>
                  <UserPlus className="mr-2 h-5 w-5 2xl:h-6 2xl:w-6" /> Professionista
                </Link>
              </Button>
              <Button asChild size="lg" variant="outline" className="border-gray-300 text-gray-900 hover:bg-gray-50 shadow-lg w-full sm:w-auto min-w-[200px] 2xl:min-w-[240px] 2xl:text-lg 2xl:px-8 2xl:py-6">
                <Link href={ROUTES.REGISTER_COMPANY}>
                  <Building className="mr-2 h-5 w-5 2xl:h-6 2xl:w-6" /> Azienda
                </Link>
              </Button>
            </div>
          </div>
